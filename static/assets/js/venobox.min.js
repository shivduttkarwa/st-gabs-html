...
document.querySelectorAll('.sg-photo-grid__grid.image-gallery').forEach(function (gallery, galleryIndex) {
    const galleryName = gallery.getAttribute('data-gall') || 'photo-grid-' + (galleryIndex + 1);
    gallery.querySelectorAll('.sg-photo-grid__item[data-src]').forEach(function (item) {
        if (item.querySelector('.sg-photo-grid__link')) return;

        const src = item.getAttribute('data-src');
        const img = item.querySelector('.sg-photo-grid__img');
        if (!src || !img) return;

        const link = document.createElement('a');
        link.href = src;
        link.className = 'venobox-link sg-photo-grid__link';
        link.setAttribute('data-gall', item.getAttribute('data-gall') || galleryName);
        link.setAttribute('data-vbtype', 'image');
        link.setAttribute('aria-label', img.getAttribute('alt') || 'Open gallery image');

        item.insertBefore(link, img);
        link.appendChild(img);

        if (!link.querySelector('.sg-photo-grid__overlay')) {
            const overlay = document.createElement('div');
            overlay.className = 'sg-photo-grid__overlay';
            overlay.innerHTML = '<i class="fa-solid fa-expand" aria-hidden="true"></i>';
...
if(venobox_link.length) {
    new VenoBox({
        ...
        onContentLoaded: function(newcontent){
            ...
            const ratioBox = document.querySelector('.venoratio');
