...
// Helper function to get video element from a slide
function getVideoFromSlide(slide) {
    if (!slide) return null;
    return slide.querySelector('video.sg-hero__bg');
}

// Helper function to play video with fallback for Video.js
function playVideo(videoEl) {
    if (!videoEl) return;
    
    // Check if it's a Video.js player
    if (videoEl.classList.contains('video-js') && window.videojs && videoEl.id) {
        const player = window.videojs.getPlayer(videoEl.id);
        if (player) {
            player.play().catch(err => console.warn('Video play error:', err));
            return;
        }
    }
...
// Helper function to pause video with fallback for Video.js
...
// Handle video playback for the current slide
function handleSlideVideoPlayback(swiper) {
    // Pause all videos
    ...
    const activeVideo = getVideoFromSlide(activeSlide);
    if (activeVideo) {
    ...
    heroSwiperInstance = new Swiper(heroSwiperEl, {
        ...
        on: {
            ...
            slideChangeTransitionEnd(swiper) {
                // Handle video playback when slide transition completes
                handleSlideVideoPlayback(swiper);
            },
        },
    });
    
    // Play video on the active slide when page completes loading
    window.addEventListener('load', () => {
        if (heroSwiperInstance) {
            // Give a small delay to ensure everything is rendered
            setTimeout(() => {
                handleSlideVideoPlayback(heroSwiperInstance);
            }, 300);
        }
    });
}
